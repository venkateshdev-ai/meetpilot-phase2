...
import { Card, Badge, Avatar, Button, TextField } from "@/components/ui";
...
// Real member management: role changes hit PATCH /api/org/members/[id]/role,
// "Add member" hits POST /api/org/members (creates a real User with whatever
// email you give it, distinct from your own, and sends them a real invite
// email via Resend with a temp password — see src/app/api/org/members/route.ts).
export default function AdminMembersPanel({
  ...
  async function handleAddMember(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setResult({ ok: false, message: body.error ?? "Could not add member" });
      setSubmitting(false);
      return;
    }
    ...
    if (body.emailSent) {
      ...
      setResult({
        ok: true,
        message: `Member created, but the invite email failed to send (${body.emailError ?? "no email provider configured"}). Temp password: ${body.tempPassword}`,
      });
    }
    setName("");
    setEmail("");
        ...
        {members.map((u) => (
          <div key={u.id} className="flex items-center justify-between p-4">
            ...
            <select
              ...
              className="rounded-lg border border-base-700 bg-base-900 px-3 py-1.5 text-xs text-slate-200 disabled:opacity-50"
            >
